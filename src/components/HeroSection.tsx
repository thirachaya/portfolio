  ...
  return (
    <section className={`flex flex-row items-center align-center justify-start w-full ${isMobile ? 'h-52' : 'h-64'} p-1`}>
      ...
      <div className={`${isMobile ? 'text-left items-start' : 'text-center items-center'} p-4 w-[70%] h-full flex flex-col justify-center gap-1`}>
        ...
        <p className={`${isMobile ? 'text-2xl' : 'text-5xl'} font-medium tracking-[0.1em] bg-clip-text text-transparent bg-gradient-to-r from-sky-700 to-emerald-500`}>Teerachaya Ngaesanthea,</p>
        <p className={`${isMobile ? 'text-md' : 'text-xl'}`}>
          I am a <span className="font-semibold bg-clip-text text-transparent bg-gradient-to-r from-sky-700 to-emerald-500">software developer</span> driven to grow into a software engineer.
        </p>
        {!isMobile && (
